...
def register_faces(face_cascade, face_id, max_count):
    ...
    gray_mirror = cv2.cvtColor(img_mirror, cv2.COLOR_BGR2GRAY)
    ...
    for (x,y,w,h) in faces:
        ...
        cv2.imwrite("/home/pi/sauron/dataset/User." + str(face_id) + '.' + str(COUNT_FACES_REGISTERED + max_count) + ".jpg", gray_mirror[y:y+h,x:x+w])
